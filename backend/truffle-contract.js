(function e(t, n, r) {
    ...
    (function(global) {
      ...
      var contract = (function(module) {
        ...
        Provider.prototype.send = function() {
          return this.provider.send.apply(this.provider,
            arguments);
        };

        Provider.prototype.sendAsync = function() {
          return this.provider.sendAsync.apply(this.provider,
            arguments);
        };
        ...
        var Utils = {
          ...
          decodeLogs: function(C, instance, logs) {
            return logs.map(function(log) {
              ...
              function partialABI(fullABI, indexed) {
                var inputs = fullABI.inputs.filter(function(
                  i) {
              ...
              var argTopics = logABI.anonymous ? copy.topics :
                copy.topics.slice(1);
              var indexedData = "0x" + argTopics.map(
                function(topics) {
                  return topics.slice(2);
                }).join("");
              var indexedParams = ethJSABI.decodeEvent(
                partialABI(logABI, true), indexedData);

              var notIndexedData = copy.data;
              var notIndexedParams = ethJSABI.decodeEvent(
                partialABI(logABI, false), notIndexedData
              );

              copy.event = logABI.name;

              copy.args = logABI.inputs.reduce(function(acc,
                current) {
          ...
          promisifyFunction: function(fn, C) {
            ...
            return function() {
              ...
              if (Utils.is_object(last_arg) && !Utils.is_big_number(
                  last_arg)) {
                tx_params = args.pop();
              }

              tx_params = Utils.merge(C.class_defaults,
                tx_params);
              ...
              return C.detectNetwork().then(function() {
                return new Promise(function(accept,
                  ...
                  var callback = function(error, tx) {
                    ...
                    var timeout = C.synchronization_timeout ||
                      240000;
          ...
          bootstrap: function(fn) {
            ...
            Object.keys(fn._static_methods).forEach(function(
              key) {
        ...
        function Contract(contract) {
          ...
          if (typeof contract == "string") {
            ...
            var contract_class = constructor.web3.eth.contract(this
              .abi);
          ...
          this.sendTransaction = Utils.synchronizeFunction(function(
            tx_params, callback) {
            ...
            constructor.web3.eth.sendTransaction.apply(
              constructor.web3.eth, [tx_params, callback]);
          }, this, constructor);
        ...
        Contract._static_methods = {
          ...
          new: function() {
            ...
            if (this.currentProvider == null) {
              throw new Error(this.contract_name +
            ...
            if (!this.unlinked_binary) {
              throw new Error(this._json.contract_name +
            ...
            return self.detectNetwork().then(function(
              ...
              var regex = /__[^_]+_+/g;
              var unlinked_libraries = self.binary.match(
                regex);

              if (unlinked_libraries != null) {
                unlinked_libraries = unlinked_libraries.map(
                  function(name) {
                    // Remove underscores
                    return name.replace(/_/g, "");
                  }).sort().filter(function(name, index,
                  arr) {
                  // Remove duplicates
              ...
              return new Promise(function(accept, reject) {
                var contract_class = self.web3.eth.contract(
                  self.abi);
                var tx_params = {};
                var last_arg = args[args.length - 1];

                // It's only tx_params if it's an object and not a BigNumber.
                if (Utils.is_object(last_arg) && !Utils
                  .is_big_number(last_arg)) {
                  tx_params = args.pop();
                }

                tx_params = Utils.merge(self.class_defaults,
                  tx_params);
                ...
                var intermediary = function(err,
                  ...
                  if (err == null && web3_instance !=
                    null && web3_instance.address !=
                    null) {
                ...
                contract_class.new.apply(contract_class,
                  args);
              });
            });
          },
          ...
          at: function(address) {
            ...
            if (address == null || typeof address != "string" ||
              address.length != 42) {
              throw new Error("Invalid address passed to " +
                this._json.contract_name + ".at(): " +
                address);
            }
            ...
            contract.then = function(fn) {
              return self.detectNetwork().then(function(
                ...
                return new Promise(function(accept,
                  reject) {
                  self.web3.eth.getCode(address,
                    function(err, code) {
                      if (err) return reject(err);

                      if (!code || new BigNumber(
                          code).eq(0)) {
                        return reject(new Error(
          ...
          deployed: function() {
            ...
            val.then = function(fn) {
              return self.detectNetwork().then(function() {
                ...
                if (self._json.networks[self.network_id] ==
                  null) {
                  throw new Error(self.contract_name +
          ...
          detectNetwork: function() {
            ...
            return new Promise(function(accept, reject) {
              ...
              self.web3.version.getNetwork(function(err,
                result) {
                if (err) return reject(err);
                ...
                var uris = Object.keys(self._json.networks)
                  .filter(function(network) {
                    return network.indexOf(
                      "blockchain://") == 0;
                  });

                var matches = uris.map(function(uri) {
                  return BlockchainUtils.matches.bind(
                    BlockchainUtils, uri, self.web3
                    .currentProvider);
                });

                Utils.parallel(matches, function(err,
                  results) {
                  if (err) return reject(err);
          ...
          link: function(name, address) {
            ...
            if (typeof name == "function") {
              ...
              if (contract.isDeployed() == false) {
                throw new Error(
                  "Cannot link contract without an address.");
              }

              this.link(contract.contract_name, contract.address);

              // Merge events so this contract knows about library's events
              Object.keys(contract.events).forEach(function(
                topic) {
                self.network.events[topic] = contract.events[
                  topic];
              });
          ...
          addProp: function(key, fn) {
            ...
            var getter = function() {
              ...
              return self._property_values[key] || fn.call(
                self);
            }
        ...
        Contract._properties = {
          ...
          network: function() {
            ...
            if (network_id == null) {
              throw new Error(this.contract_name +
                ...
                this.contract_name + ".setNetwork(), run " +
                this.contract_name +
          ...
          address: {
            get: function() {
              ...
              if (address == null) {
                throw new Error(
                  "Cannot find deployed address: " + this.contract_name +
            ...
            set: function(val) {
              if (val == null) {
                throw new Error(
                  "Cannot set deployed address; malformed value: " +
                  val);
              }
          ...
          binary: function() {
            ...
            Object.keys(this.links).forEach(function(
              library_name) {
              var library_address = self.links[library_name];
              var regex = new RegExp("__" + library_name +
                "_*", "g");

              binary = binary.replace(regex,
  ...
  2: [function(require, module, exports) {
    ...
    contract.fromSolJS = function(soljs_abstraction,
      ...
      Object.keys(soljs_abstraction.all_networks).forEach(function(
        ...
        if (network_name == "default") {
          if (ignore_default_network == true) {
            ...
            throw new Error(soljs_abstraction.contract_name +
        ...
        if (soljs_abstraction.all_networks[network_name].updated_at >
          ...
          latest_network_updated_at = soljs_abstraction.all_networks[
            network_name].updated_at;
        }

        networks[network_name] = {};

        ["address", "events", "links", "updated_at"].forEach(
          function(key) {
            networks[network_name][key] = soljs_abstraction.all_networks[
              network_name][key];
          })
      });
  ...
  3: [function(require, module, exports) {
    ...
    function toByteArray(b64) {
      ...
      for (i = 0, j = 0; i < l; i += 4, j += 3) {
        tmp = (revLookup[b64.charCodeAt(i)] << 18) | (revLookup[b64.charCodeAt(
            i + 1)] << 12) | (revLookup[b64.charCodeAt(i + 2)] << 6) |
      ...
      if (placeHolders === 2) {
        tmp = (revLookup[b64.charCodeAt(i)] << 2) | (revLookup[b64.charCodeAt(
          i + 1)] >> 4)
        arr[L++] = tmp & 0xFF
      } else if (placeHolders === 1) {
        tmp = (revLookup[b64.charCodeAt(i)] << 10) | (revLookup[b64.charCodeAt(
          i + 1)] << 4) | (revLookup[b64.charCodeAt(i + 2)] >> 2)
        arr[L++] = (tmp >> 8) & 0xFF
        arr[L++] = tmp & 0xFF
      }
    ...
    function tripletToBase64(num) {
      return lookup[num >> 18 & 0x3F] + lookup[num >> 12 & 0x3F] + lookup[
        num >> 6 & 0x3F] + lookup[num & 0x3F]
    }
    ...
    (function(module, exports) {
      ...
      BN.isBN = function isBN(num) {
        ...
        return num !== null && typeof num === 'object' &&
          num.constructor.wordSize === BN.wordSize && Array.isArray(
            num.words);
      };
      ...
      BN.prototype._initNumber = function _initNumber(number, base,
        ...
        if (number < 0x4000000) {
          ...
          this.words = [
            number & 0x3ffffff, (number / 0x4000000) & 0x3ffffff
          ];
          this.length = 2;
        } else {
      ...
      BN.prototype._initArray = function _initArray(number, base,
        ...
        if (endian === 'be') {
          for (i = number.length - 1, j = 0; i >= 0; i -= 3) {
            w = number[i] | (number[i - 1] << 8) | (number[i - 2] <<
              16);
      ...
      BN.prototype._parseBase = function _parseBase(number, base, start) {
        ...
        for (var limbLen = 0, limbPow = 1; limbPow <= 0x3ffffff; limbPow *=
          base) {
          limbLen++;
        }
        limbLen--;
      ...
      */
      ...
      var groupBases = [
        0, 0,
        33554432, 43046721, 16777216, 48828125, 60466176, 40353607,
        16777216,
        43046721, 10000000, 19487171, 35831808, 62748517, 7529536,
        11390625,
        16777216, 24137569, 34012224, 47045881, 64000000, 4084101,
        5153632,
        6436343, 7962624, 9765625, 11881376, 14348907, 17210368,
        20511149,
        24300000, 28629151, 33554432, 39135393, 45435424, 52521875,
        60466176
      ];
      ...
      BN.prototype.toArrayLike = function toArrayLike(ArrayType, endian,
        length) {
        var byteLength = this.byteLength();
        var reqLength = length || Math.max(1, byteLength);
        assert(byteLength <= reqLength,
      ...
      BN.prototype.inotn = function inotn(width) {
        ...
        // Handle the residue
        if (bitsLeft > 0) {
          this.words[i] = ~this.words[i] & (0x3ffffff >> (26 -
            bitsLeft));
        }
      ...
      FFTM.prototype.permute = function permute(rbt, rws, iws, rtws,
        itws, N) {
        for (var i = 0; i < N; i++) {
          rtws[i] = rws[rbt[i]];
          itws[i] = iws[rbt[i]];
      ...
      FFTM.prototype.transform = function transform(rws, iws, rtws,
        itws, N, rbt) {
      ...
      BN.prototype.divRound = function divRound(num) {
        ...
        // Round up
        return dm.div.negative !== 0 ? dm.div.isubn(1) : dm.div.iaddn(
          1);
      };
      ...
      Red.prototype._verify2 = function _verify2(a, b) {
        assert((a.negative | b.negative) === 0,
      ...
      Mont.prototype.imul = function imul(a, b) {
        ...
        var t = a.imul(b);
        var c = t.maskn(this.shift).mul(this.minv).imaskn(this.shift)
          .mul(this.m);
  ...
  6: [function(require, module, exports) {
    ...
    function alloc(size, fill, encoding) {
      ...
      if (fill !== undefined) {
        ...
        return typeof encoding === 'string' ? createBuffer(size).fill(
          fill, encoding) : createBuffer(size).fill(fill)
      }
      return createBuffer(size)
    }
    ...
    Buffer.prototype.equals = function equals(b) {
      if (!Buffer.isBuffer(b)) throw new TypeError(
    ...
    Buffer.prototype.write = function write(string, offset, length,
      ...
      if (length === undefined || length > remaining) length =
        remaining
    ...
    function utf8Slice(buf, start, end) {
      ...
      while (i < end) {
        var firstByte = buf[i]
        var codePoint = null
        var bytesPerSequence = (firstByte > 0xEF) ? 4 : (firstByte > 0xDF) ?
          3 : (firstByte > 0xBF) ? 2 : 1
    ...
    function checkOffset(offset, ext, length) {
      if ((offset % 1) !== 0 || offset < 0) throw new RangeError(
        'offset is not uint')
      if (offset + ext > length) throw new RangeError(
    ...
    function checkInt(buf, value, offset, ext, max, min) {
      ...
      if (value > max || value < min) throw new RangeError(
        '"value" argument is out of bounds')
      if (offset + ext > buf.length) throw new RangeError(
        'Index out of range')
    }
    ...
    (function(Buffer) {
      ...
      function encodeParams(types, values) {
        if (types.length !== values.length) {
          throw new Error(
            ...
            types.length + ' should be ' + values.length);
        }
      ...
      function encodeMethod(method, values) {
        var signature = method.name + '(' + utils.getKeys(method.inputs,
          'type').join(',') + ')';
        var signatureEncoded = '0x' + new Buffer(utils.keccak256(
          signature), 'hex').slice(0, 4).toString('hex');
        var paramsEncoded = encodeParams(utils.getKeys(method.inputs,
          'type'), values).substring(2);
      ...
      function decodeMethod(method, data) {
        ...
        return decodeParams(outputNames, outputTypes, utils.hexOrBuffer(
          data));
      }
      ...
      function decodeEvent(eventObject, data) {
        ...
        return decodeParams(inputNames, inputTypes, utils.hexOrBuffer(
          data));
      }
    ...
    (function(Buffer) {
      ...
      function hexOrBuffer(valueInput, name) {
        ...
        if (!Buffer.isBuffer(value)) {
          if (!isHexString(value)) {
            var error = new Error(name ? '[ethjs-abi] invalid ' + name :
      ...
      function coderNumber(size, signed) {
        return {
          encode: function encodeNumber(valueInput) {
            ...
            if (typeof value === 'object' && value.toString && (value
                .toTwos || value.dividedToIntegerBy)) {
          ...
          decode: function decodeNumber(data, offset) {
            ...
            var value = new BN(data.slice(offset + junkLength, offset +
      ...
      function coderFixedBytes(length) {
        return {
          ...
          decode: function decodeFixedBytes(data, offset) {
            ...
            return {
              consumed: 32,
              value: '0x' + data.slice(offset, offset + length).toString(
                'hex')
            };
          }
        };
      ...
      var coderAddress = {
        ...
        decode: function decodeAddress(data, offset) {
          ...
          return {
            consumed: 32,
            value: '0x' + data.slice(offset + 12, offset + 32).toString(
              'hex')
          };
        }
      };
      ...
      function encodeDynamicBytesHelper(value) {
        ...
        return Buffer.concat([uint256Coder.encode(value.length), value,
          padding
        ]);
      }
      ...
      function decodeDynamicBytesHelper(data, offset) {
        if (data.length < offset + 32) {
          throw new Error(
            ...
            data.length + ' should be less than ' + (offset + 32));
        }
          ...
          encode: function encodeArray(value) {
            ...
            if (length !== value.length) {
              throw new Error(
                ...
                length + ' does not equal ' + value.length);
            }

            value.forEach(function(resultValue) {
              result = Buffer.concat([result, coder.encode(
                resultValue)]);
            });
            ...
            case 'uint':
              ...
              if (intSize === 0 || intSize > 256 || intSize % 8 !== 0) {
                throw new Error(
                  ...
                  type + ', invalid ' + prefix + '<N> width: ' + type
                );
              }
  ...
  11: [function(require, module, exports) {
    (function(process, global) {
      ...
      (function(root) {
        ...
        var COMMON_JS = !root.JS_SHA3_TEST && typeof module ==
        ...
        var SHIFT = [0, 8, 16, 24];
        var RC = [1, 0, 32898, 0, 32906, 2147483648, 2147516416,
          2147483648, 32907, 0, 2147483649,
          0, 2147516545, 2147483648, 32777, 2147483648, 138, 0, 136,
          0, 2147516425, 0,
          2147483658, 0, 2147516555, 0, 139, 2147483648, 32905,
          2147483648, 32771,
        ...
        var createShakeOutputMethod = function(bits, padding,
          outputType) {
          return function(message, outputBits) {
            return new Keccak(bits, padding, outputBits).update(
              message)[outputType]();
          }
        };
        ...
        var createShakeMethod = function(bits, padding) {
          ...
          for (var i = 0; i < OUTPUT_TYPES.length; ++i) {
            var type = OUTPUT_TYPES[i];
            method[type] = createShakeOutputMethod(bits, padding,
              type);
          }
          return method;
        };
        ...
        for (var i = 0; i < algorithms.length; ++i) {
          ...
          for (var j = 0; j < bits.length; ++j) {
            methods[algorithm.name + '_' + bits[j]] = algorithm.createMethod(
              bits[j], algorithm.padding);
          }
        }
        ...
        Keccak.prototype.update = function(message) {
          ...
          while (index < length) {
            ...
            if (notString) {
              for (i = this.start; index < length && i < byteCount; ++
                index) {
              ...
              for (i = this.start; index < length && i < byteCount; ++
                ...
                if (code < 0x80) {
                  ...
                  blocks[i >> 2] |= (0xc0 | (code >> 6)) << SHIFT[i++
                    & 3];
                  blocks[i >> 2] |= (0x80 | (code & 0x3f)) << SHIFT[
                    i++ & 3];
                } else if (code < 0xd800 || code >= 0xe000) {
                  blocks[i >> 2] |= (0xe0 | (code >> 12)) << SHIFT[
                    i++ & 3];
        ...
        Keccak.prototype.toString = Keccak.prototype.hex = function() {
          ...
          while (j < outputBlocks) {
            for (i = 0; i < blockCount && j < outputBlocks; ++i, ++
              j) {
              block = s[i];
              hex += HEX_CHARS[(block >> 4) & 0x0F] + HEX_CHARS[
                  block & 0x0F] +
                HEX_CHARS[(block >> 12) & 0x0F] + HEX_CHARS[(block >>
                  8) & 0x0F] +
                HEX_CHARS[(block >> 20) & 0x0F] + HEX_CHARS[(block >>
                  16) & 0x0F] +
                HEX_CHARS[(block >> 28) & 0x0F] + HEX_CHARS[(block >>
                  24) & 0x0F];
            }
            if (j % blockCount == 0) {
              f(s);
          ...
          if (extraBytes) {
            ...
            if (extraBytes > 1) {
              hex += HEX_CHARS[(block >> 12) & 0x0F] + HEX_CHARS[(
                block >> 8) & 0x0F];
            }
            if (extraBytes > 2) {
              hex += HEX_CHARS[(block >> 20) & 0x0F] + HEX_CHARS[(
                block >> 16) & 0x0F];
            }
          }
          return hex;
        ...
        var f = function(s) {
          var h, l, n, c0, c1, c2, c3, c4, c5, c6, c7, c8, c9,
            ...
            b29, b30, b31, b32, b33,
            b34, b35, b36, b37, b38, b39, b40, b41, b42, b43, b44,
            b45, b46, b47, b48, b49;
          for (n = 0; n < 48; n += 2) {
  ...
  12: [function(require, module, exports) {
    ...
    module.exports = function numberToBN(arg) {
      if (typeof arg === 'string' || typeof arg === 'number') {
        ...
        if ((!stringArg.match(/^-?[0-9]+$/) && stringArg.match(
            /^[0-9A-Fa-f]+$/)) || stringArg.match(/^[a-fA-F]+$/) || (
      ...
      } else if (typeof arg === 'object' && arg.toString && (!arg.pop &&
          !arg.push)) {
      ...
      throw new Error('[number-to-bn] while converting number ' + JSON.stringify(
          arg) +
    ...
    var Blockchain = {
      ...
      asURI: function(provider, callback) {
        ...
        web3.eth.getBlock(0, function(err, genesis) {
          ...
          web3.eth.getBlock("latest", function(err, latest) {
            ...
            var url = "blockchain://" + genesis.hash.replace(
              "0x", "") + "/block/" + latest.hash.replace(
              "0x", "");
      ...
      matches: function(uri, provider, callback) {
        ...
        web3.eth.getBlock(0, function(err, block) {
          if (err) return callback(err);
          if (block.hash != expected_genesis) return callback(
            null, false);
  ...
  16: [function(require, module, exports) {
    ...
    var TruffleSchema = {
      ...
      generateBinary: function(options, existing_binary, extra_options) {
        ...
        if (existing_binary.unlinked_binary && existing_binary.unlinked_binary
          .indexOf("0x") < 0) {
        ...
        Object.keys(options.networks).forEach(function(network_id) {
          existing_binary.networks[network_id] = options.networks[
            network_id];
        });
        ...
        if (options.network_id) {
          ...
          existing_binary.abi.forEach(function(item) {
            ...
            var signature = item.name + "(" + item.inputs.map(
              function(param) {
                return param.type;
              }).join(",") + ")";
      ...
      var CryptoJS = CryptoJS || (function(Math, undefined) {
        ...
        var Base = C_lib.Base = (function() {
          ...
          return {
            ...
            extend: function(overrides) {
              ...
              if (!subtype.hasOwnProperty('init') || this.init ===
                subtype.init) {
        ...
        var WordArray = C_lib.WordArray = Base.extend({
          ...
          concat: function(wordArray) {
            ...
            if (thisSigBytes % 4) {
              ...
              for (var i = 0; i < thatSigBytes; i++) {
                var thatByte = (thatWords[i >>> 2] >>> (24 -
                  (i % 4) * 8)) & 0xff;
                thisWords[(thisSigBytes + i) >>> 2] |=
                  thatByte << (24 - ((thisSigBytes + i) % 4) *
                    8);
              }
            } else {
          ...
          clamp: function() {
            ...
            // Clamp
            words[sigBytes >>> 2] &= 0xffffffff << (32 - (
              sigBytes % 4) * 8);
          ...
          random: function(nBytes) {
            ...
            var r = (function(m_w) {
              ...
              return function() {
                m_z = (0x9069 * (m_z & 0xFFFF) + (m_z >>
                  0x10)) & mask;
                m_w = (0x4650 * (m_w & 0xFFFF) + (m_w >>
                  0x10)) & mask;
                var result = ((m_z << 0x10) + m_w) &
                  mask;
                result /= 0x100000000;
                result += 0.5;
                return result * (Math.random() > .5 ?
                  1 : -1);
              }
            });

            for (var i = 0, rcache; i < nBytes; i += 4) {
              var _r = r((rcache || Math.random()) *
                0x100000000);
        ...
        var Hex = C_enc.Hex = {
          ...
          parse: function(hexStr) {
            ...
            for (var i = 0; i < hexStrLength; i += 2) {
              words[i >>> 3] |= parseInt(hexStr.substr(i, 2),
                16) << (24 - (i % 8) * 4);
            }
        ...
        var Latin1 = C_enc.Latin1 = {
          ...
          parse: function(latin1Str) {
            ...
            for (var i = 0; i < latin1StrLength; i++) {
              words[i >>> 2] |= (latin1Str.charCodeAt(i) &
                0xff) << (24 - (i % 4) * 8);
            }
        ...
        var Utf8 = C_enc.Utf8 = {
          ...
          parse: function(utf8Str) {
            return Latin1.parse(unescape(encodeURIComponent(
              utf8Str)));
          }
        };
          ...
          Base.extend({
            ...
            _process: function(doFlush) {
              ...
              if (doFlush) {
                ...
                nBlocksReady = Math.max((nBlocksReady | 0) -
              ...
              // Count bytes ready
              var nBytesReady = Math.min(nWordsReady * 4,
                dataSigBytes);

              // Process blocks
              if (nWordsReady) {
                for (var offset = 0; offset < nWordsReady; offset +=
                  blockSize) {
                ...
                var processedWords = dataWords.splice(0,
                  nWordsReady);
                data.sigBytes -= nBytesReady;
              }

              // Return processed words
              return new WordArray.init(processedWords,
                nBytesReady);
            },
  ...
  18: [function(require, module, exports) {;
    (function(root, factory, undef) {
      if (typeof exports === "object") {
        // CommonJS
        module.exports = exports = factory(require("./core"), require(
          "./x64-core"));
      } else if (typeof define === "function" && define.amd) {
        // AMD
      ...
      (function(Math) {
        ...
        var SHA3 = C_algo.SHA3 = Hasher.extend({
          ...
          _doReset: function() {
            ...
            this.blockSize = (1600 - 2 * this.cfg.outputLength) /
              32;
          },
          ...
          _doProcessBlock: function(M, offset) {
            ...
            for (var round = 0; round < 24; round++) {
              ...
              for (var x = 0; x < 5; x++) {
                ...
                var tMsw = Tx4.high ^ ((Tx1Msw << 1) | (
                  Tx1Lsw >>> 31));
                var tLsw = Tx4.low ^ ((Tx1Lsw << 1) | (Tx1Msw >>>
                  31));
                for (var y = 0; y < 5; y++) {
                  var lane = state[x + 5 * y];
                  lane.high ^= tMsw;
              ...
              for (var x = 0; x < 5; x++) {
                for (var y = 0; y < 5; y++) {
                  ...
                  // Mix rows
                  lane.high = TLane.high ^ (~Tx1Lane.high &
                    Tx2Lane.high);
                  lane.low = TLane.low ^ (~Tx1Lane.low &
                    Tx2Lane.low);
                }
              }
          ...
          _doFinalize: function() {
            ...
            // Add padding
            dataWords[nBitsLeft >>> 5] |= 0x1 << (24 -
              nBitsLeft % 32);
            dataWords[((Math.ceil((nBitsLeft + 1) /
                blockSizeBits) * blockSizeBits) >>> 5) - 1] |=
              0x80;
            ...
            return new WordArray.init(hashWords,
              outputLengthBytes);
          },
